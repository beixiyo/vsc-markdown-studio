...
function DemoSection({ title, mode, color }: { title: string, mode: 'scroll' | 'intersection', color: string }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-[320px] bg-backgroundPrimary rounded-xl border border-border overflow-hidden shadow-xs">
      <div className="p-4 border-b border-border flex justify-between items-center bg-background2/20">
        ...
        <span className={ cn('px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider', color) }>
          { mode }
        </span>
      </div>
