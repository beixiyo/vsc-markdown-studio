...
export async function runHeadingHoverTest() {
  ...
  MDTest.asyncTestCase(R, '标题获取功能测试', async () => {
    ...
    if (firstHeading.level !== 1) return false
    if (firstHeading.text !== '一级标题') return false

    // 测试第二个段落的上级标题（应该是二级标题）
    const secondParagraph = docs[3] // 第四个块是段落
    const secondHeading = MDBridge.getParentHeading(secondParagraph.id)

    if (!secondHeading) return false
    if (secondHeading.level !== 2) return false
    if (secondHeading.text !== '二级标题') return false
