...
export async function runCommandsTest() {
  ...
  MDTest.testCase(R, '1.1 setHeading(1)', () => {
    ...
    window.MDBridge!.command.setHeading(1)
  ...
  MDTest.testCase(R, '2.1 setParagraph()', () => {
    MDTest.clearContent()
    window.MDBridge!.setContent([{ type: 'heading', content: '测试标题', props: { level: 1 } }])
    window.MDBridge!.command.setParagraph()
    return { success: true }
  })
  ...
  MDTest.testCase(R, '3.1 setUnorderedList()', () => {
    MDTest.clearContent()
    window.MDBridge!.setContent([{ type: 'paragraph', content: '测试段落' }])
    window.MDBridge!.command.setUnorderedList()
    const doc = window.MDBridge!.getDocument()
  ...
  MDTest.testCase(R, '3.2 setOrderedList()', () => {
    MDTest.clearContent()
    window.MDBridge!.setContent([{ type: 'paragraph', content: '测试段落' }])
    window.MDBridge!.command.setOrderedList()
    const doc = window.MDBridge!.getDocument()
  ...
  MDTest.testCase(R, '4.1 setBold()', () => {
    MDTest.clearContent()
    window.MDBridge!.setContent([{ type: 'paragraph', content: '测试段落' }])
    window.MDBridge!.command.setBold()
    return { success: true }
  })
  ...
  MDTest.testCase(R, '4.2 setItalic() - 测试命令执行', () => {
    ...
    window.MDBridge!.setContent([{ type: 'paragraph', content: '测试段落' }])
    /** 测试命令是否能正常执行（不检查结果） */
    window.MDBridge!.command.setItalic()
    return { success: true }
  })
  ...
  MDTest.testCase(R, '5.1 setCheckList()', () => {
    MDTest.clearContent()
    window.MDBridge!.setContent([{ type: 'paragraph', content: '测试段落' }])
    window.MDBridge!.command.setCheckList()
    const doc = window.MDBridge!.getDocument()
