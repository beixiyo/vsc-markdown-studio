  ...
  useEffect(() => {
    ...
    const bridge: MDBridge = {
      ...
      aiEdit: {
        ...
        showLoadingFrame: payload => regionEdit.showLoadingFrame(payload),
        hideLoadingFrame: (id, options) => regionEdit.hideLoadingFrame(id, options),
        accept: options => withNativeContentChange('ai-edit-accept', true, () => regionEdit.accept(options)),
