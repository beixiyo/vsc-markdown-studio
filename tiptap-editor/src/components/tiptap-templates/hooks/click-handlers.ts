 ...
 */
const handleMetaLinkOpen = (event: MouseEvent) => {
 ...
 * 判断点击位置是否落在当前选区范围
 */
const isWithinSelection = (clickPos: { pos: number }, from: number, to: number) =>
  clickPos.pos >= from && clickPos.pos <= to

/**
 * 当点击命中现有选区时，移动光标到点击位置以取消选区
 */
export const handleSelectionToggle = (view: EditorView, clickPos: { pos: number }) => {
 ...
 */
export const createHandleClick = () => (view: EditorView, _pos: number, event: Event) => {
